import { ArrowLeft, Mic2, Volume2, Save, Wand2, AudioLines, Share2, RefreshCw } from "lucide-react";
import { useState, useEffect } from "react";
import { useLocation } from "wouter";
import { databases } from "../../lib/appwrite"; // ✅ Appwrite Import
...
export default function VoiceLab() {
  ...
  // 🛡️ 1. DYNAMIC API INJECTION (Fetch Voice Engine Key)
  useEffect(() => {
  ...
  const handleSynthesize = async () => {
    if (!script) return;
    if (!voiceApiKey) {
      alert("⚠️ Voice Engine Offline: Please inject VOICE_LAB key in Hub!");
      return;
    }

    setIsSynthesizing(true);
    // 🚀 Actual API Call (e.g. ElevenLabs) goes here using voiceApiKey
    
    setTimeout(() => setIsSynthesizing(false), 4000); // Simulation
  };
    ...
    <div className="min-h-screen bg-[#050505] text-white p-6 pb-24 font-sans selection:bg-purple-500/30">
      ...
      <header className="flex items-center justify-between mb-10">
        ...
        <div className="text-center">
          <h2 className="text-sm font-black italic uppercase tracking-[0.3em] text-purple-400">RX Voice Lab</h2>
      ...
      {/* Visualizer Area */}
      <div className="bg-zinc-900/40 border border-white/5 rounded-[45px] p-12 mb-8 flex flex-col items-center justify-center relative overflow-hidden group">
        ...
        <p className={`text-[9px] font-black uppercase tracking-[0.4em] transition-colors ${isSynthesizing ? 'text-purple-400 animate-pulse' : 'text-zinc-600'}`}>
          {isSynthesizing ? "Processing Vocal Patterns..." : "Neural Voice Ready"}
        </p>
      </div>
      ...
      <div className="bg-zinc-900/20 border border-white/5 p-10 rounded-[50px] shadow-2xl">
        ...
        <textarea 
          ...
          placeholder="Type or paste the dialogue for the AI to speak..."
