...
export default function Dashboard() {
  ...
  // ✅ CONSTANT: AAPKA ACTUAL RENDER URL
  const API_BASE = "https://race-x-nexus.onrender.com"; 

  useEffect(() => {
    account.get().then(setUser).catch(() => console.log("Nexus Offline"));
    // Server ko pehle hi jaga dene ke liye ping
    fetch(API_BASE).catch(() => console.log("Waking up core..."));
  }, []);

  // ===== LOGIC: REAL API CALLS =====
  const executeProtocol = async (endpoint: string, type: 'video' | 'audio' | 'image') => {
    try {
      ...
      alert("Render server so raha hai. Ek baar 30-40 seconds rukk kar firse button dabayein.");
    } finally {
      setLoading(false);
  ...
  return (
    <div className="h-screen w-screen bg-black text-white flex flex-col relative overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col items-center justify-center z-10 px-6 pb-10">
        ...
        <p className="text-zinc-500 text-[10px] font-black mb-10 tracking-[0.4em] uppercase opacity-60">SELECT PROTOCOL</p>
        ...
        {(loading || status) && (
          <div className="mt-10 flex flex-col items-center gap-2">
            <div className="w-20 h-[1px] bg-zinc-800 overflow-hidden">
        ...
        {resultVideo && (
          <div className="mt-8 w-full max-w-md animate-in zoom-in-95 duration-500">
             <div className="relative p-1 rounded-2xl bg-gradient-to-b from-cyan-400/20 to-transparent shadow-2xl">
                <video src={resultVideo} controls autoPlay className="w-full rounded-xl border border-white/5" />
             </div>
             <button onClick={() => window.open(resultVideo)} className="w-full mt-4 bg-white text-black py-4 rounded-xl font-black text-[10px] tracking-widest uppercase active:scale-95 transition-all">
                📥 Download Production
             </button>
          </div>
        )}
...
// ===== 3D TILT BUTTON (Wahi Screenshot Wala Style) =====
