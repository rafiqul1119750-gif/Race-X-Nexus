...
export default function Dashboard() {
  ...
  return (
    <div className="p-6 bg-black min-h-screen text-white flex flex-col items-center">
      ...
      {/* Grid using Link component (Highly Stable) */}
      <div className="grid grid-cols-2 gap-6 w-full max-w-md px-2">
        {menu.map((item) => (
          <Link key={item.path} href={item.path}>
            <a className="aspect-square bg-zinc-900/40 border border-white/5 rounded-[45px] flex flex-col items-center justify-center gap-4 active:scale-90 transition-all group cursor-pointer">
              <div className={`w-16 h-16 rounded-[24px] flex items-center justify-center ${item.color} text-black shadow-lg group-hover:scale-105 transition-transform`}>
      ...
      {/* Footer Status */}
      <div className="mt-auto w-full max-w-sm p-5 bg-zinc-950/50 border border-white/5 rounded-[35px] flex items-center justify-between">
